    ...
    this.prompt = ChatPromptTemplate.fromMessages([
      ...
      5. **高效执行复合任务 (IMPORTANT)**：
         ...
         - **地理精确性 (CRITICAL)**：
           ...
           - 若因工具调用次数限制无法逐一规划所有路段，应如实告知用户，并建议他们使用地图APP查看完整路线。
         - **路线结果呈现 (IMPORTANT)**：
           - 呈现路线时只需给出**距离、时间、推荐交通方式**，不要逐步列出每个转弯方向。
           - 步行超过2km或30分钟时，自动建议公交/打车替代方案。步行超过5km时标注"不适合步行"。
           - 示例格式："🚶 **樱花园**：步行约500米，约8分钟。也可骑行约3分钟。"
           - 多个目的地时用表格风格对比呈现（距离、时间、推荐方式），不要逐个大段描述。
         - **工具调用数量限制**：单次回复中，同一个工具最多调用 3 次。优先规划最关键的 2-3 段路线，其余用定性描述（如"距离较近，步行可达"），不要编造具体数值。
